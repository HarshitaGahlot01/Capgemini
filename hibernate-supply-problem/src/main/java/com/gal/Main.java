...
public class Main {
	public static void main(String[] args) {
        ...
        try {
            ...
            List<String> Q1 =em.createQuery("select s.sname from Supplier s", String.class).getResultList();
            System.out.println("Supplier Names:");
            for(String name : Q1) {
                System.out.println(name);
            }
            List<Supplier> Q2 =em.createQuery("from Supplier s where s.city='Delhi'", Supplier.class).getResultList();
            System.out.println("Suppliers from Delhi:");
            for(Supplier s : Q2) {
                System.out.println(s.getSname());
            }
            List<String> Q3 = em.createQuery("select sp.item.name from Supply sp where sp.supplier.sid=1",String.class) .getResultList();
